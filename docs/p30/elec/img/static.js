...
escope: (sel) => {
    ...
    g.rect([10, 10], [0, 5]);
    let leaf = svg.group({fill: "silver"}, "black1");
    g.line([-2, 8], [2, 8]);
...
induct: (sel) => {
    ...
    let plus = svg.pm(2.4, 1).css("black3", "black").config({shift: [-7, 0]});
    plus.rect([4, 4]).css({fill: "none"});
    let red = svg.group("red", ".Toggle0");
    red.gtext("Electron Flow", ["text", "f22", "nostroke"], [4.25, 5]);
    let g = svg.group("black1").config({shift: [1, 0]});
    g.line([2.5, 0], [4.5, 0]);
    let circ = g.group("black3", "nofill");
    circ.circle(2.5);
    circ.circle(2.5, [7, 0]);
...
pole: (sel) => {
    ...
    let circ = g.group("black3", "nofill");
    circ.circle(2.5);
    let chg = g.group("text", "f20", "black", "nostroke");
    let t = 2;
    for (let a=-1; a<2; a++) {
