...
pend: (sel) => {
    ...
    svg.tick_label(1, 0, [...range(0, 3.6, 0.5)], "-6").find("g.LabelY").config({shift: ["-20", 0]}).$.find(".Zero").remove();
    ...
    model.locus2(x => 0.965 * x + 0.947, [0, 2.5]).$.hide().addClass("Toggle0");
    model.locus2(x => twoPi * Math.sqrt(x / 9.81), [0, 2.5]).$.hide().addClass("Toggle1");
    p.plot({x:x, y:y}, "5");
...
pend_lin: (sel) => {
    ...
    let y = [1.01, 1.43, 1.74, 2.01, 2.24, 2.45, 2.65, 2.85, 3.0];
    let p = svg.group();
    let line = p.group(".Locus", "none", "#0065fe@2").locus2(x => 2.01 * x, [0, 1.75]).$.hide();
    p.plot({x:x, y:y}, "5");
