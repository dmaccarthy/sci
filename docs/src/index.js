...
home.crumbs = id => {
    ...
    if (id != "home") {
        ...
        for (let item of id.split("/")) {
            ...
            p.append(" / ");
            let title = pg.title.split('#');
            let a = $("<a>").html(title[title.length - 1].trim()).appendTo(p);
            a.attr("href", "#" + path);
        }
    }
...
home.go = (id) => {
    ...
    fetch("./" + id + ".htm?_" + (new Date().getTime())).then(r => {
...
page.unpublish = art => {
    ...
    else for (let e of art.find("[data-answers]")) {
        e = $(e);
        let a = e.attr("data-answers");
        if (a != "1") {
            a = new Date(a);
            if (a > t) e.find(".Answer").remove();
        }
    }
}
...
page.onload = (id, args) => {
    ...
    // Show location
    document.title = $("#MainTitle").html(feed.title.replace('#', '')).text();
    home.crumbs(id);
...
page.show_post = n => {
    let i = 0;
    $("#MainTitle").html(page.get("title").replace('#', ''));
...
page._get_icon = item => {
    let icon = item.icon;
    return icon ? page.icon(icon) : page._get_icon(item._parent);
}
...
page.menu = (u, menu) => {
    ...
    let add = table ? item => {
        ...
        $("<td>").appendTo(tr).html(page._get_icon(item));
        tr.append($("<td>").html(item.title.replace('#', '')));
    } : item => {
