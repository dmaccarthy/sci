...
car: (sel) => {
    ...
    svg.symb(0, ["F", 1], ["→", 5, [0, "20"]], ["g", 6, ["12", "-8"]]).align([0.15, -0.25]).css("symbol", "red", "f28");
...
stick: (sel) => {
    ...
    g = svg.group().css("symbol", "red", "f28");
...
book: (sel) => {
    ...
    let books = svg.group().config({theta: a}).css("black1", {fill: "#32cd32"});
    books.rect(["20", "5"], [0, -1]);
    ...
    g.css("symbol", "f28", {stroke: "none"}).align([0.15, -0.75]);
},
