...
lightbulb: (sel, linear) => {
    ...
    svg.graph({grid: [0.25, 0.25], css: true, appendAxes: 1,
    ...
    let x0 = linear ? -0.2 : -0.3;
    svg.delay(svg.group().config({theta: 90}), {recenter: [x0, 1.45]}).text("2").css({"font-size": "14px"});
    if (linear) svg.delay(svg.group(), {recenter: [1.36, -0.2]}).text("–2").css({"font-size": "14px"});
